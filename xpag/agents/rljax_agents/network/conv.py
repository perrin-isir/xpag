...
from xpag.agents.rljax_agents.network.initializer import DeltaOrthogonal
...
class SACDecoder(hk.Module):
    ...
    def __call__(self, x):
        ...
        w_init = hk.initializers.Orthogonal(
            scale=np.sqrt(2 / (1 + self.negative_slope**2))
        )
        x = hk.Linear(self.last_conv_dim, w_init=w_init)(x)
        x = nn.leaky_relu(x, self.negative_slope).reshape(
