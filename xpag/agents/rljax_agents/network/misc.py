...
from xpag.agents.rljax_agents.network.conv import DQNBody, SLACDecoder, SLACEncoder
...
class Gaussian(hk.Module):
    ...
    def __call__(self, x):
        x = MLP(
            ...
            hidden_activation=partial(
                nn.leaky_relu, negative_slope=self.negative_slope
            ),
        )(x)
...
def make_stochastic_latent_variable_model(
    ...
    def fn_z1_prior(z2, a):
        return Gaussian(output_dim=z1_dim, hidden_units=units_model)(
            jnp.concatenate([z2, a], axis=1)
        )

    def fn_z1_post(f, z2, a):
        return Gaussian(output_dim=z1_dim, hidden_units=units_model)(
            jnp.concatenate([f, z2, a], axis=1)
        )

    def fn_z2(z1, z2, a):
        return Gaussian(output_dim=z2_dim, hidden_units=units_model)(
            jnp.concatenate([z1, z2, a], axis=1)
        )

    def fn_reward(z_, a_, n_z_):
        x = jnp.concatenate([z_, a_, n_z_], axis=-1)
        B, S, X = x.shape
        mean, std = Gaussian(output_dim=1, hidden_units=units_model)(
            x.reshape([B * S, X])
        )
    ...
    def fn_decoder(z1_, z2_):
        return SLACDecoder(state_space=state_space, std=np.sqrt(0.1))(
            jnp.concatenate([z1_, z2_], axis=-1)
        )

    network_dict = {}
    params_dict = {}

    network_dict["z1_prior_init"] = hk.without_apply_rng(
        hk.transform(lambda x: ConstantGaussian(z1_dim, 1.0)(x))
    )
    params_dict["z1_prior_init"] = network_dict["z1_prior_init"].init(
        next(rng), fake_action
    )

    network_dict["z1_prior"] = hk.without_apply_rng(hk.transform(fn_z1_prior))
    params_dict["z1_prior"] = network_dict["z1_prior"].init(
        next(rng), fake_z2, fake_action
    )

    network_dict["z1_post_init"] = hk.without_apply_rng(
        hk.transform(lambda x: Gaussian(z1_dim, units_model)(x))
    )
    params_dict["z1_post_init"] = network_dict["z1_post_init"].init(
        next(rng), fake_feature
    )

    network_dict["z1_post"] = hk.without_apply_rng(hk.transform(fn_z1_post))
    params_dict["z1_post"] = network_dict["z1_post"].init(
        next(rng), fake_feature, fake_z2, fake_action
    )

    network_dict["z2_init"] = hk.without_apply_rng(
        hk.transform(lambda x: Gaussian(z2_dim, units_model)(x))
    )
    params_dict["z2_init"] = network_dict["z2_init"].init(next(rng), fake_z1)

    network_dict["z2"] = hk.without_apply_rng(hk.transform(fn_z2))
    params_dict["z2"] = network_dict["z2"].init(
        next(rng), fake_z1, fake_z2, fake_action
    )

    network_dict["reward"] = hk.without_apply_rng(hk.transform(fn_reward))
    params_dict["reward"] = network_dict["reward"].init(
        next(rng), fake_z_, fake_action_, fake_z_
    )
