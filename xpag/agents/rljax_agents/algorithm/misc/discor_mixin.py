...
from xpag.agents.rljax_agents.network import ContinuousQFunction
from xpag.agents.rljax_agents.util import load_params, save_params
...
class DisCorMixIn:
    ...
    def _calculate_weight_list(
        ...
        next_error_list = self._calculate_error_list(
            params_actor, params_error_target, next_state, key
        )
        weight_list = []
        for next_error, rm_error in zip(next_error_list, rm_error_list):
            x = -(1.0 - done) * self.gamma * next_error / rm_error
            weight_list.append(
                jax.lax.stop_gradient(jax.nn.softmax(x, axis=0) * x.shape[0])
            )
        return weight_list
    ...
    def _loss_error(
        ...
        for error, next_error, abs_td in zip(error_list, next_error_list, abs_td_list):
            target = jax.lax.stop_gradient(
                abs_td + (1.0 - done) * self.gamma * next_error
            )
    ...
    def load_params(self, save_dir):
        self.params_error = self.params_error_target = load_params(
            os.path.join(save_dir, "params_error.npz")
        )
