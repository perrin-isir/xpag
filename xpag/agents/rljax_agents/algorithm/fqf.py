...
class FQF(QRDQN):
    ...
    def __init__(
        ...
        if setup_net:
            ...
            self.net, self.params, fake_feature = make_quantile_nerwork(
                self.rng, state_space, action_space, fn, num_quantiles
            )
        ...
        self.cum_p_net = hk.without_apply_rng(
            hk.transform(lambda s: CumProbNetwork(num_quantiles=num_quantiles)(s))
        )
        self.params_cum_p = self.cum_p_net.init(next(self.rng), fake_feature)
        opt_init, self.opt_cum_p = optix.rmsprop(
            lr_cum_p, decay=0.95, eps=1e-5, centered=True
        )
    ...
    def _forward_from_feature(
        ...
        quantile_s = self.net["quantile"].apply(
            params["quantile"], feature, cum_p_prime
        )
    ...
    def _calculate_value(
        ...
        return get_quantile_at_action(
            self.net["quantile"].apply(params["quantile"], feature, cum_p), action
        )
    ...
    def _calculate_target(
        ...
        if self.double_q:
            next_action = self._forward_from_feature(
                params_cum_p, params, next_feature
            )[:, None]
        else:
            next_action = self._forward_from_feature(
                params_cum_p, params_target, next_feature
            )[:, None]
        next_quantile = self._calculate_value(
            params_target, next_feature, next_action, cum_p_prime
        )
    ...
    def _loss(
        ...
        cum_p_prime = jax.lax.stop_gradient(
            self.cum_p_net.apply(params_cum_p, feature)[1]
        )
        quantile = self._calculate_value(params, feature, action, cum_p_prime)
        target = self._calculate_target(
            params_cum_p, params, params_target, reward, done, next_feature, cum_p_prime
        )
    ...
    def _loss_cum_p(self, params_cum_p, params, state, action):
        feature = jax.lax.stop_gradient(
            self.net["feature"].apply(params["feature"], state)
        )
        cum_p, cum_p_prime = self.cum_p_net.apply(params_cum_p, feature)
        quantile = get_quantile_at_action(
            self.net["quantile"].apply(params["quantile"], feature, cum_p[:, 1:-1]),
            action,
        )
        quantile_prime = get_quantile_at_action(
            self.net["quantile"].apply(params["quantile"], feature, cum_p_prime), action
        )
        # NOTE: Proposition 1 in the paper requires F^{-1} is non-decreasing.
        # I relax this requirements and calculate gradients of taus even
        # when F^{-1} is not non-decreasing.
        val1 = quantile - quantile_prime[:, :-1]
        sign1 = quantile > jnp.concatenate(
            [quantile_prime[:, :1], quantile[:, :-1]], axis=1
        )
        val2 = quantile - quantile_prime[:, 1:]
        sign2 = quantile < jnp.concatenate(
            [quantile[:, 1:], quantile_prime[:, -1:]], axis=1
        )
