...
from xpag.agents.rljax_agents.network import DiscreteImplicitQuantileFunction
from xpag.agents.rljax_agents.util import fake_state
...
class IQN(QRDQN):
    ...
    def __init__(
        ...
        if not hasattr(self, "fake_args"):
            self.fake_args = (
                ...
                np.empty((1, num_quantiles), dtype=np.float32),
            )
    ...
    def _calculate_target(
        ...
        next_quantile = self._calculate_value(
            params_target, next_state, next_action, cum_p=cum_p_prime
        )
    ...
    def _loss(
        ...
        cum_p_prime = jax.random.uniform(
            key_list[1], (state.shape[0], self.num_quantiles)
        )
        quantile = self._calculate_value(params, state, action, cum_p=cum_p)
        target = self._calculate_target(
            params, params_target, reward, done, next_state, cum_p_prime, key_list[2]
        )
