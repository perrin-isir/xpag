...
from xpag.agents.rljax_agents.network import DiscreteQuantileFunction
from xpag.agents.rljax_agents.util import get_quantile_at_action, quantile_loss
...
class QRDQN(DQN):
    ...
    def __init__(
        ...
        if self.name == "QR-DQN":
            self.cum_p_prime = jnp.expand_dims(
                (jnp.arange(0, num_quantiles, dtype=jnp.float32) + 0.5) / num_quantiles,
                0,
            )
    ...
    def _calculate_value(
        ...
        **kwargs,
        ...
        return get_quantile_at_action(
            self.net.apply(params, state, *args, **kwargs), action
        )
    ...
    def _calculate_target(
        ...
        **kwargs,
        ...
        if self.double_q:
            ...
            next_action = self._forward(params_target, next_state, *args, **kwargs)[
                ..., None
            ]
    ...
    def _calculate_loss_and_abs_td(
        ...
        return loss, jax.lax.stop_gradient(
            jnp.abs(td).sum(axis=1).mean(axis=1, keepdims=True)
        )
    ...
    def _loss(
        ...
        return self._calculate_loss_and_abs_td(
            quantile, target, self.cum_p_prime, weight
        )
