...
class SAC_AE(SAC):
    ...
    def __init__(
        ...
        self.params_encoder = self.params_encoder_target = self.encoder.init(
            next(self.rng), fake_state(state_space)
        )
        ...
        self.decoder = hk.without_apply_rng(
            hk.transform(
                lambda x: SACDecoder(state_space, num_filters=32, num_layers=4)(x)
            )
        )
    ...
    def update(self, writer=None):
        ...
        if self.learning_step % self.update_interval_target == 0:
            self.params_encoder_target = self._update_target_ae(
                self.params_encoder_target, self.params_encoder
            )
            self.params_linear_target = self._update_target_ae(
                self.params_linear_target, self.params_linear
            )
    ...
    def _loss_critic(
        ...
        **kwargs,
        ...
        next_last_conv = jax.lax.stop_gradient(
            self.encoder.apply(params_critic["encoder"], next_state)
        )
    ...
    def _loss_actor(
        ...
        **kwargs,
        ...
        last_conv = jax.lax.stop_gradient(
            self.encoder.apply(params_critic["encoder"], state)
        )
    ...
    def _loss_ae(
        ...
        return (
            ...
            + self.lambda_weight * loss_weight,
            None,
        )
    ...
    def load_params(self, save_dir):
        ...
        self.params_encoder = self.params_encoder_target = load_params(
            os.path.join(save_dir, "params_encoder.npz")
        )
        self.params_linear = self.params_linear_target = load_params(
            os.path.join(save_dir, "params_linear.npz")
        )
