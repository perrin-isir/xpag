...
from xpag.agents.rljax_agents.algorithm.base_class import QLearning
from xpag.agents.rljax_agents.network import DiscreteQFunction
from xpag.agents.rljax_agents.util import get_q_at_action, huber, optimize
...
class DQN(QLearning):
    ...
    def __init__(
        ...
        if setup_net:
            ...
            self.params = self.params_target = self.net.init(
                next(self.rng), *self.fake_args
            )
    ...
    def _calculate_target(
        ...
        if self.double_q:
            ...
            next_q = jnp.max(
                self.net.apply(params_target, next_state), axis=-1, keepdims=True
            )
