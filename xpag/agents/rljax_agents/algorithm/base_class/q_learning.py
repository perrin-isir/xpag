...
from xpag.agents.rljax_agents.algorithm.base_class.base_algorithm import (
    OffPolicyAlgorithm,
)
from xpag.agents.rljax_agents.util import fake_state, load_params, save_params
...
class QLearning(OffPolicyAlgorithm):
    ...
    def _calculate_target(
        self, params, params_target, reward, done, next_state, *args, **kwargs
    ):
        pass
    ...
    def kwargs_update(self):
        return (
            {"key_list": self.get_key_list(self.num_keys_loss)}
            if (self.num_keys_loss)
            else {}
        )
    ...
    def load_params(self, save_dir):
        self.params = self.params_target = load_params(
