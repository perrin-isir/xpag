...
from xpag.agents.rljax_agents.network import CategoricalPolicy, DiscreteQFunction
from xpag.agents.rljax_agents.util import get_q_at_action
...
class SAC_Discrete(SAC):
    ...
    def _calculate_value_list(
        ...
        return [
            get_q_at_action(q_s, action)
            for q_s in self.critic.apply(params_critic, state)
        ]
    ...
    def _calculate_value(
        ...
        q_s = jax.lax.stop_gradient(
            jnp.asarray(self.critic.apply(params_critic, state)).min(axis=0)
        )
